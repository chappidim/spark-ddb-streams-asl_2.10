...
private[kinesis] class KinesisReceiver[T](
  ...
  override def onStart() {
    ...
    */
    ...
    //Now with StreamConfig
    val adapterClient = new AmazonDynamoDBStreamsAdapterClient(awsCredProvider, new ClientConfiguration())
    adapterClient.setEndpoint(endpointUrl)
    val dynamoDBClient = new AmazonDynamoDBClient(awsCredProvider, new ClientConfiguration())
    dynamoDBClient.setEndpoint("https://dynamodb.us-west-2.amazonaws.com")
    val cloudWatchClient = new AmazonCloudWatchClient(awsCredProvider, new ClientConfiguration())
    
    worker = new Worker(recordProcessorFactory, kinesisClientLibConfiguration, adapterClient, dynamoDBClient, cloudWatchClient)
